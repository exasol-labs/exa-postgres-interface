...
**Current release:** `v0.0.8` — see
[Releases](https://github.com/nconforti93/exa-postgres-interface/releases).
v0.0.8 translates PostgreSQL `SESSION_USER` to Exasol `CURRENT_USER`, softens
...
```bash
curl -LO https://github.com/nconforti93/exa-postgres-interface/releases/download/v0.0.8/exa-postgres-interface-v0.0.8-linux-x86_64.tar.gz
curl -LO https://github.com/nconforti93/exa-postgres-interface/releases/download/v0.0.8/exa-postgres-interface-v0.0.8-linux-x86_64.tar.gz.sha256
sha256sum -c exa-postgres-interface-v0.0.8-linux-x86_64.tar.gz.sha256
tar -xzf exa-postgres-interface-v0.0.8-linux-x86_64.tar.gz
```
...
```bash
exa-postgres-interface-v0.0.8-linux-x86_64/bin/exa-postgres-interface \
...
```bash
exa-postgres-interface-v0.0.8-linux-x86_64/bin/exasol_exec \
  --dsn EXASOL_HOST:8563 \
  --user sys \
  --password 'EXASOL_PASSWORD' \
  --file exa-postgres-interface-v0.0.8-linux-x86_64/sql/postgres_catalog_compatibility.sql
```
...
```bash
sudo install -m 0755 exa-postgres-interface-v0.0.8-linux-x86_64/bin/exa-postgres-interface /opt/exa-postgres-interface/bin/exa-postgres-interface
sudo install -m 0640 -o root -g exa-postgres-interface exa-postgres-interface-v0.0.8-linux-x86_64/config/example.toml /etc/exa-postgres-interface/config.toml
sudo install -m 0644 exa-postgres-interface-v0.0.8-linux-x86_64/packaging/exa-postgres-interface.service /etc/systemd/system/exa-postgres-interface.service
